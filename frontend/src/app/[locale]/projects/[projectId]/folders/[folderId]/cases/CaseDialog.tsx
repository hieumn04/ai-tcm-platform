...
type Props = {
  isOpen: boolean
  onCancel: () => void
  onSubmit: (customId: string, title: string, description: string) => void
...
export default function CaseDialog({
  ...
  const clear = () => {
    setCustomId({ text: '', isValid: false, errorMessage: '' })
    setCaseName({
      isValid: false,
      text: 'Untitled Case',
      errorMessage: '',
  ...
  const validate = () => {
    ...
    onSubmit(customId.text, caseTitle.text, caseDescription.text)
    clear()
  }
      ...
      <ModalContent>
        ...
        <ModalBody>
          <Input
            ...
            label="ID"
            value={customId.text}
            isInvalid={customId.isValid}
            errorMessage={customId.errorMessage}
            onChange={(e) => {
              setCustomId({ ...customId, text: e.target.value })
            }}
          />
          <Input
            type="text"
