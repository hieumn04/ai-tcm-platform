...
export default function WeeklyLineChart({ projectId }: Props) {
  const [showGrid, setShowGrid] = useState(true)
  const [showFill, setShowFill] = useState(true)
  const chartRef = useRef<ChartJS<'line'>>(null)
  const chartInstanceId = useMemo(
    () => `weekly-chart-${Math.random().toString(36).substring(2, 9)}`,
    [],
  )

  const { chartData, loading, error } = useContextAggregatedChartData('weekly')
  ...
  const handleDownloadChart = useCallback(() => {
    if (chartRef.current) {
      ...
      link.download = `weekly-cases-chart-${new Date().toISOString().split('T')[0]}.png`
