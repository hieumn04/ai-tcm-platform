...
export default function MonthlyLineChart({ projectId }: Props) {
  const [showGrid, setShowGrid] = useState(true)
  const [showFill, setShowFill] = useState(true)
  const chartRef = useRef<ChartJS<'line'>>(null)
  const chartInstanceId = useMemo(
    () => `monthly-chart-${Math.random().toString(36).substring(2, 9)}`,
    [],
  )

  const { chartData, loading, error } = useContextAggregatedChartData('monthly')
  ...
  const handleDownloadChart = useCallback(() => {
    if (chartRef.current) {
      ...
      link.download = `monthly-cases-chart-${new Date().toISOString().split('T')[0]}.png`
