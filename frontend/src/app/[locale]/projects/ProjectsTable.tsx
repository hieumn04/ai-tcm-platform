...
export default function ProjectsTable({ projects, messages, locale }: Props) {
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...projectsArray].sort((a: ProjectType, b: ProjectType) => {
        ...
        <TableBody emptyContent={messages.noProjectsFound} items={sortedItems}>
          {(item) => (
            <TableRow
              key={item.id}
              onClick={() =>
                (window.location.href = `/${locale}/projects/${item.id}/home`)
              }
              className="cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-800 transition"
            >
              {(columnKey) => (
